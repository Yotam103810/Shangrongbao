...
public class HttpUtils {
    ...
    public static byte[] send(String strUrl,String reqmethod,byte[] reqData){
        try{
            ...
            httpcon.setDoOutput(true);  //允许发送数据（输出）
            httpcon.setDoInput(true);  //允许接收数据（输入）
            httpcon.setUseCaches(false);  //不使用缓存
            httpcon.setConnectTimeout(CONN_TIMEOUT); //链接超时时间
            httpcon.setReadTimeout(READ_TIMEOUT);  //读取超时时间
            httpcon.setRequestMethod(reqmethod);  //设置请求方法
            httpcon.connect(); //建立与服务器链接
            //reqmethod.equalsIgnoreCase(POST)忽略大小写比较
